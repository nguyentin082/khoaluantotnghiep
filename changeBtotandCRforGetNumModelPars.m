...
if reduce_overhead
    ...
    coeff = func_gram_schmidt(coeff(:,1:400));% "V^Np" trực giao hóa các cột từ 1 đến 500 của coeff bằng cách sử dụng hàm func_gram_schmidt
    fprintf('Calculating Parameters of PCA...\n');
    num_dft_coeffs = na * nc / CR;
    N_O_pca = 2 * num_dft_coeffs * size(coeff, 2) + num_dft_coeffs * size(coeff, 2) + 2 * (na * nc);
    disp(['Number of offloaded parameters (N_O^{pca}): ', num2str(N_O_pca)]);
else
    coeff = coeff_ori;% khong reduce parameters
    fprintf('Calculating Parameters of PCA... (NOT BE REDUCED)\n');
    N_O_pca = 2 * (na * nc  * size(coeff, 2)) + 2 * (na * nc);
    disp(['Number of offloaded parameters (N_O^{pca}): ', num2str(N_O_pca)]);
end
...
if quantization
    ...
    b1 = max(Bs);
    % Calculate the number of offloaded parameters
    N_O_kmeans = (2 * sum(2.^(1:b1))) + size(coeff, 2) + BTot;
...
disp(['Number of ALL offloaded parameters: ', num2str(N_O)]);